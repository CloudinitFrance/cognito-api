...
def check_inputs(req_body):
    '''Validate inputs'''
    return assert_valid_schema(req_body, constants.CONFIRM_MFA_JSON_SCHEMA)
...
def confirm_otp(user_id, email, otp, conf_values):
    '''Confirm the new user'''
    s3 = boto3.client('s3')

    tmp_filename = '/tmp/' + user_id + '.png'
    s3.download_file(
        conf_values['S3_BUCKET_MFA_BUCKET'],
        'users_mfa/' + user_id + '.png',
        tmp_filename
    )
    qrcode_image = cv2.imread(tmp_filename)
    detector = cv2.QRCodeDetector()
    data, vertices_array, binary_qrcode = detector.detectAndDecode(qrcode_image)
    if vertices_array is not None:
        otp_secret = data.split('secret=')[1].split('&')[0]
...
def build_api_response(email, user_id):
    ...
    response_body['mfa_status'] = 'CONFIRMED'
...
def lambda_handler(event, _):
    ...
    try:
        ...
        if not is_otp_valid:
            return {
                'statusCode': 401,
                'body': json.dumps({'error_message': 'Cannot confirm the otp, contact the support'}),
                'headers': {
                    'Content-Type' : 'application/json',
                    'Access-Control-Allow-Origin' : '*',
                    'Allow' : 'POST, OPTIONS',
