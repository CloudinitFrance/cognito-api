...
def check_inputs(req_body):
    '''Validate inputs'''
    return assert_valid_schema(req_body, constants.RESET_PASSWORD_JSON_SCHEMA)


def reset_password(email, conf_values):
    '''Reset user password'''
    try:
        cup_client = boto3.client('cognito-idp', conf_values['REGION'])
        cup_client.admin_reset_user_password(
            UserPoolId=conf_values['COGNITO_USER_POOL_ID'],
            Username=email
        )
        return True
...
def build_api_response(email, user_id):
    ...
    response_body['status'] = 'PASSWORD_RESET_CONFIRMATION_SENT'
...
def lambda_handler(event, _):
    ...
    try:
        ...
        if user_id is None:
            return {
                'statusCode': 401,
                'body': json.dumps({'error_message': 'Cannot reset the password, contact the support'}),
                'headers': {
                    'Content-Type' : 'application/json',
                    'Access-Control-Allow-Origin' : '*',
                    'Allow' : 'POST, OPTIONS',
        ...
        response_reset_password = reset_password(
            req_body['email'],
            conf_values
        )
        if not response_reset_password:
            return {
                'statusCode': 401,
                'body': json.dumps({'error_message': 'Cannot reset the password, contact the support'}),
                'headers': {
