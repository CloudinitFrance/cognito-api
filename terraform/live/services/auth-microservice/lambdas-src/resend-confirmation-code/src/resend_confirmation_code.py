#! /usr/bin/env python
'''Resend new user confirmation code'''
...
def check_inputs(req_body):
    '''Validate inputs'''
    return assert_valid_schema(req_body, constants.RESEND_CONFIRMATION_CODE_JSON_SCHEMA)


def resend_confirmation_code(email, conf_values):
    '''Resend confirmation code'''
    try:
        cup_client = boto3.client('cognito-idp', conf_values['REGION'])
        response_challenge = cup_client.admin_create_user(
            UserPoolId=conf_values['COGNITO_USER_POOL_ID'],
            Username=email,
            MessageAction='RESEND',
            DesiredDeliveryMediums=['EMAIL']
        )
        return response_challenge['User']['Username']
...
def build_api_response(email, user_id):
    ...
    response_body['status'] = 'CONFIRMATION_CODE_SENT'
...
def lambda_handler(event, _):
    ...
    try:
        ...
        if user_id is None:
            return {
                'statusCode': 401,
                'body': json.dumps({'error_message': 'Cannot resend the confirmation code, contact the support'}),
                'headers': {
                    'Content-Type' : 'application/json',
                    'Access-Control-Allow-Origin' : '*',
                    'Allow' : 'POST, OPTIONS',
        ...
        response_resend_confirmation_code = resend_confirmation_code(
            req_body['email'].lower(),
            conf_values
        )
        if response_resend_confirmation_code is None:
            return {
                'statusCode': 401,
                'body': json.dumps({'error_message': 'Cannot resend the confirmation code, contact the support'}),
                'headers': {
